...
public class managerLimitController {
	...
	@FXML
	private ChoiceBox<SpecialAccount> choiceBox;

	public void initialize() {
		ArrayList<Customer> customers = ((Manager) loginController.loggedUser).getCustomerList();
		for (int i = 0; i < customers.size(); i++) {
			choiceBox.getItems().addAll(customers.get(i).getSpecialAccountsList());
		}
		...
		inputLimit.textProperty().addListener((observable, oldValue, newValue) -> {
			if (newValue.isEmpty())
				return;

			if (!newValue.matches(regex)) {
				inputLimit.setText(oldValue);
			}
		});
	}
	...
	public void onItemSelected() {
		inputLimit.managedProperty().set(true);
	...
	protected void onSaveClick(ActionEvent actionEvent) throws IOException {
		...
		String message;
		try {
			limitAmount = Double.parseDouble(inputLimit.getText().replace(',', '.'));
			message = "Limit saved.";

			choiceBox.getValue().setLimit(limitAmount);
			System.out.println(choiceBox.getValue().getLimit());
